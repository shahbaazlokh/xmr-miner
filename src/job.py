	...
	def mine(self, client, miner, thread): 
		if client.message is not None:
			if 'params' in client.message:
				...
				self.height = client.message['params']['height']
			else:
				...
				self.target = b''.join([self.target[i:i+2] for i in range(0, len(self.target), 2)][::-1])
				self.height = client.message['result']['job']['height']
		else:
			return
		...
		pow.argtypes = [ctypes.POINTER(ctypes.c_char), ctypes.c_int, ctypes.POINTER(ctypes.c_char), ctypes.c_int, ctypes.c_int, ctypes.c_ulong]
		...
		for nonce in range(0x00000000, 0x7fffffff, 0x00000001):
			...
			pow(input_bin, 76, output_bin, miner.variant, 0, self.height)
